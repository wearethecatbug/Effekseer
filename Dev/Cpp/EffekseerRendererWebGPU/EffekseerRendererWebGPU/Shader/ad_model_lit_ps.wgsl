// Effekseer Advanced Model Lit Pixel Shader (WGSL)
// Converted from ad_model_lit_ps.fx.frag
...
fn main(input: PSInput) -> @location(0) vec4<f32> {
    ...
    // Normal map
    var normalTexValue = textureSample(normalTex, texSampler, input.uv_others.xy + uvOffset).xyz;
    normalTexValue = (normalTexValue - vec3<f32>(0.5)) * 2.0;
    let tangentSpace = mat3x3<f32>(input.worldT, input.worldB, input.worldN);
    let localNormal = normalize(tangentSpace * normalTexValue);
    ...
    Output = vec4<f32>(Output.xyz * (cb.fLightColor.xyz * diffuse + cb.fLightAmbient.xyz), Output.w);
    
    // Falloff
    if (cb.fFalloffParameter.x == 1.0) {
        let cameraVec = normalize(-cb.fCameraFrontDirection.xyz);
        let CdotN = clamp(dot(cameraVec, localNormal), 0.0, 1.0);
